...
主要额度的剩余比例默认常驻在菜单栏图标旁。App Server 返回 `usedPercent`；Codex Helper 会计算 `100 - usedPercent` 并限制在 0–100%，显示方向与 Codex 官方界面一致。点击一次即可在一级菜单中看到全部额度周期。重新设计的主页面和可选桌面悬浮小组件共用同一份额度快照、重置倒计时与颜色分级：50–100% 为蓝绿色，10–50% 为琥珀色，低于 10% 为珊瑚红。无边框小组件会浮在普通窗口之上，可以出现在所有桌面空间、自由拖动，并提供刷新、打开主页面和隐藏操作。显示状态仅保存在本机，默认关闭。
