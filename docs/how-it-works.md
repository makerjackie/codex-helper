...
The primary remaining percentage is shown beside the menu bar icon by default. The App Server reports `usedPercent`; Codex Helper presents `100 - usedPercent`, clamped to 0–100%, to match the direction used by Codex itself. Clicking once reveals every available quota window directly in the first-level menu. The redesigned dashboard and optional floating desktop widget use the same snapshot, reset countdown, and color bands: blue-green at 50–100%, amber at 10–50%, and coral-red below 10%. The borderless widget stays above normal windows, can join every Space, is draggable, and provides refresh, dashboard, and hide controls. Its visibility is stored locally and defaults to off.
